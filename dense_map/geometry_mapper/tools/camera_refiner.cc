...
struct DepthToHazError {
  ...
  DepthToHazError(Eigen::Vector2d const& haz_pix, Eigen::Vector3d const& depth_xyz, std::vector<int> const& block_sizes,
...
void calc_median_residuals(std::vector<double> const& residuals,
  ...
  void add_haz_nav_cost(// Inputs                                                            // NOLINT
    ...
    if (haz_cam_intensity_timestamps[haz_it] >=
        sparse_map_timestamps[nav_cam_start + nav_it] + navcam_to_hazcam_timestamp_offset) {
      match_left = true;
    } else {
    ...
    if (nav_cam_start + right_nav_it >= static_cast<int>(sparse_map_timestamps.size()) ||
    ...
    double left_time = sparse_map_timestamps[nav_cam_start + left_nav_it]
      + navcam_to_hazcam_timestamp_offset;
    double right_time = sparse_map_timestamps[nav_cam_start + right_nav_it]
      + navcam_to_hazcam_timestamp_offset;
    ...
    Eigen::Affine3d left_nav_trans = nav_cam_affines[nav_cam_start + left_nav_it];
    Eigen::Affine3d right_nav_trans = nav_cam_affines[nav_cam_start + right_nav_it];
    ...
    for (size_t ip_it = 0; ip_it < haz_ip_vec.size(); ip_it++) {
      ...
      problem.AddResidualBlock(depth_to_nav_cost_function, depth_to_nav_loss_function,
                               &nav_cam_vec[left_nav_index],
                               &nav_cam_vec[right_nav_index],
      ...
      if (FLAGS_fix_map) {
        problem.SetParameterBlockConstant(&nav_cam_vec[left_nav_index]);
        problem.SetParameterBlockConstant(&nav_cam_vec[right_nav_index]);
      }
    }
  ...
  // Prevent the linter from messing up with the beautiful formatting below
  void add_haz_sci_cost(// Inputs                                                            // NOLINT
    ...
    for (size_t ip_it = 0; ip_it < haz_ip_vec.size(); ip_it++) {
      ...
      problem.AddResidualBlock(depth_to_sci_cost_function, depth_to_sci_loss_function,
                               &nav_cam_vec[NUM_RIGID_PARAMS * (nav_cam_start + left_nav_it)],
                               &nav_cam_vec[NUM_RIGID_PARAMS * (nav_cam_start + right_nav_it)],
      ...
      if (FLAGS_fix_map) {
        problem.SetParameterBlockConstant(&nav_cam_vec[NUM_RIGID_PARAMS * (nav_cam_start + left_nav_it)]);
        problem.SetParameterBlockConstant(&nav_cam_vec[NUM_RIGID_PARAMS * (nav_cam_start + right_nav_it)]);
      }
    }
  ...
  void add_nav_sci_cost(// Inputs                                                            // NOLINT
    ...
    double left_time = sparse_map_timestamps[nav_cam_start + left_nav_it] + navcam_to_hazcam_timestamp_offset;
    double right_time = sparse_map_timestamps[nav_cam_start + right_nav_it] + navcam_to_hazcam_timestamp_offset;
    double sci_time = sci_cam_timestamps[sci_it] + scicam_to_hazcam_timestamp_offset;
    ...
    for (size_t ip_it = 0; ip_it < sci_ip_vec.size(); ip_it++) {
      ...
      problem.AddResidualBlock(nav_cost_function, nav_loss_function,
                               &nav_cam_vec[(nav_cam_start + nav_it) * NUM_RIGID_PARAMS],
      ...
      if (FLAGS_fix_map)
        problem.SetParameterBlockConstant(&nav_cam_vec[(nav_cam_start + nav_it)
                                                       * NUM_RIGID_PARAMS]);
  ...
  // Use these to keep track where in the bags we are. After one traversal forward
    ...
    for (size_t map_it = 0; map_it + 1 < sparse_map_timestamps.size(); map_it++) {
      if (FLAGS_start >= 0.0 && FLAGS_duration > 0.0) {
        ...
        if (sparse_map_timestamps[map_it] + navcam_to_hazcam_timestamp_offset
            < FLAGS_start + haz_cam_start_time &&
            sparse_map_timestamps[map_it] < FLAGS_start + haz_cam_start_time)
          continue;
      }
      ...
      if (std::abs(sparse_map_timestamps[map_it + 1] - sparse_map_timestamps[map_it]) > FLAGS_bracket_len) continue;
      ...
      dense_map::pickTimestampsInBounds(all_haz_cam_inten_timestamps,
                                        sparse_map_timestamps[map_it],
                                        sparse_map_timestamps[map_it + 1],
                                        -navcam_to_hazcam_timestamp_offset,
      ...
      for (size_t samp_it = 0; samp_it < local_haz_timestamps.size(); samp_it++) {
        ...
        double nav_start = sparse_map_timestamps[map_it] + navcam_to_hazcam_timestamp_offset
          - haz_cam_start_time;
      ...
      dense_map::pickTimestampsInBounds(all_sci_cam_timestamps, sparse_map_timestamps[map_it],
                                        sparse_map_timestamps[map_it + 1],
    ...
    if (!stop_early) {
      images.push_back(cv::Mat());
      if (!dense_map::lookupImage(sparse_map_timestamps.back(), nav_cam_handle.bag_msgs,
                                  save_grayscale, images.back(),
                                  nav_cam_pos, found_time))
        LOG(FATAL) << "Cannot look up nav cam image at given time.";
      cid_to_image_type.push_back(dense_map::NAV_CAM);
    ...
    if (images.size() > sparse_map_timestamps.size() + haz_cam_intensity_timestamps.size()
  ...
  void detect_match_features(// Inputs                                            // NOLINT
    ...
    for (size_t pair_it = 0; pair_it < image_pairs.size(); pair_it++) {
      ...
      thread_pool2.AddTask(&dense_map::matchFeatures, &match_mutex, left_image_it, right_image_it,
                           cid_to_descriptor_map[left_image_it],
                           cid_to_descriptor_map[right_image_it],
                           cid_to_keypoint_map[left_image_it], cid_to_keypoint_map[right_image_it],
...
int main(int argc, char** argv) {
  ...
  sparse_map_timestamps.resize(sparse_map_images.size());
  ...
  if (sparse_map_timestamps.empty()) LOG(FATAL) << "No sparse map timestamps found.";
  ...
  std::vector<Eigen::Affine3d>& nav_cam_affines = sparse_map->cid_to_cam_t_global_;  // alias

  if (FLAGS_start >= 0.0 && FLAGS_duration > 0.0) {
    double latest_start
      = sparse_map_timestamps.back() + navcam_to_hazcam_timestamp_offset - haz_cam_start_time;
    if (latest_start < FLAGS_start) {
      std::cout << "The sparse map ended before the desired start time. "
                << "Use a start time no later than " << latest_start << "." << std::endl;
      return 1;
    }

    double earliest_end = sparse_map_timestamps[0] + navcam_to_hazcam_timestamp_offset
      - haz_cam_start_time;
    if (earliest_end > FLAGS_start + FLAGS_duration) {
      std::cout << "The sparse map did not start yet. The sum of start and duration "
                << "must be at least " << earliest_end << "." << std::endl;
      return 1;
    }
  }
  ...
  select_images_to_match(  // Inputs                        // NOLINT
    ...
    scicam_to_hazcam_timestamp_offset,                      // NOLINT
    sparse_map_timestamps, all_haz_cam_inten_timestamps,    // NOLINT
  ...
  // Put the sparse map camera transforms in a vector so we can optimize them further
  int num_cams = nav_cam_affines.size();
  std::vector<double> nav_cam_vec(num_cams * dense_map::NUM_RIGID_PARAMS);
  for (int cid = 0; cid < num_cams; cid++)
    dense_map::rigid_transform_to_array(nav_cam_affines[cid],
                                        &nav_cam_vec[dense_map::NUM_RIGID_PARAMS * cid]);
  ...
  for (auto it = matches.begin(); it != matches.end(); it++) {
    ...
    if (image_haz_it != image_to_haz_it.end() && image_nav_it != image_to_nav_it.end()) {
      ...
      dense_map::add_haz_nav_cost(// Inputs                                             // NOLINT
                                  ...
                                  sparse_map_timestamps, haz_cam_to_left_nav_cam_index, // NOLINT
                                  ...
                                  nav_cam_affines, depth_clouds,                        // NOLINT
                                  // Outputs                                            // NOLINT
                                  residual_names, hazcam_depth_to_image_scale,          // NOLINT
                                  nav_cam_vec, hazcam_to_navcam_vec,                    // NOLINT
    ...
    if (image_haz_it != image_to_haz_it.end() && image_sci_it != image_to_sci_it.end()) {
      ...
      add_haz_sci_cost(  // Inputs                                                       // NOLINT
                       ...
                       haz_cam_intensity_timestamps, sparse_map_timestamps,              // NOLINT
                       ...
                       depth_to_haz_block_sizes, nav_cam_affines,                        // NOLINT
                       depth_clouds,                                                     // NOLINT
                       // Outputs                                                        // NOLINT
                       residual_names, hazcam_depth_to_image_scale, nav_cam_vec,         // NOLINT
    ...
    if (image_nav_it != image_to_nav_it.end() && image_sci_it != image_to_sci_it.end()) {
      ...
      add_nav_sci_cost(// Inputs                                                      // NOLINT
                       ...
                       scicam_to_hazcam_timestamp_offset,                             // NOLINT
                       match_pair,                                                    // NOLINT
                       sparse_map_timestamps, sci_cam_timestamps,                     // NOLINT
                       ...
                       nav_cam_affines, depth_clouds, mesh,                           // NOLINT
                       bvh_tree,                                                      // NOLINT
                       // Outputs                                                     // NOLINT
  ...
  for (size_t pid = 0; pid < sparse_map->pid_to_cid_fid_.size(); pid++) {
    ...
    for (auto cid_fid = sparse_map->pid_to_cid_fid_[pid].begin();
      ...
      if (FLAGS_fix_map)
        problem.SetParameterBlockConstant(&nav_cam_vec[dense_map::NUM_RIGID_PARAMS * cid]);
    }
  }
  ...
  dense_map::array_to_rigid_transform(scicam_to_hazcam_aff_trans, &scicam_to_hazcam_vec[0]);
  for (int cid = 0; cid < num_cams; cid++)
    dense_map::array_to_rigid_transform(nav_cam_affines[cid], &nav_cam_vec[dense_map::NUM_RIGID_PARAMS * cid]);
