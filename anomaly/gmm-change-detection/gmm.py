...
class GMM:
    ...
    def remove_gaussian(self, index):    # Index of gaussian to remove
        ...
        removed_gauss = GMM(self.weights[index], self.means[index,:].reshape((1,3)), np.array(self.covariances[:,:,index].reshape((3,3))))
    ...
    def add_gaussian(self, gaussian):   # Gaussian in form of GMM, k=1
        self.weights = np.append(self.weights, gaussian.weights)    # PLACEHOLDER (weights are now incorrect)
        ...
        if type(self.covariances) is not np.ndarray:
            ...
            self.covariances = np.concatenate([self.covariances, gaussian.covariances], axis=0)
        self.n_gaussians = self.weights.size
