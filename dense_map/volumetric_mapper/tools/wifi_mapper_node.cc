...
namespace wifi_mapper {

class WifiMapperNode{
 public:
  WifiMapperNode() {
  }
  ...
  void Initialize(ros::NodeHandle* nh) {
    ...
    config_params_.AddFile("dense_map/wifi_mapper.config");
    ...
    // Wifi Subscribers to the same callback
    sub_  = nh->subscribe(TOPIC_HARDWARE_WIFI, 100,
                      &WifiMapperNode::WifiCallback, this);
    // Wifi publishers at a lower rate
    timer_pub_ = nh->createTimer(ros::Duration(20),
                      &WifiMapperNode::WifiPubCallback, this, false, false);
  }
 ...
 protected:
  ...
  void WifiCallback(const isaac_hw_msgs::WifiSignals::ConstPtr& msg) {
    ...
    for (int i = 0; i < msg->signals.size(); ++i) {
      ...
      if (calc_trace_) {
        ...
        if (trace_maps_.find(msg->signals[i].ssid) == trace_maps_.end()) {
          ROS_DEBUG_STREAM("New trace map " << msg->signals[i].ssid <<
            " topic " << TOPIC_WIFI_MAPPER_TRACE + std::string("_") + msg->signals[i].ssid);
          volumetric_mapper::TraceMapper new_map_trace(nh_,
              TOPIC_WIFI_MAPPER_TRACE + std::string("_") + msg->signals[i].ssid,
              resolution_, min_signal_strength_, max_signal_strength_, alpha_trace_);
          trace_maps_.insert({msg->signals[i].ssid, new_map_trace});
        }
        trace_maps_.find(msg->signals[i].ssid)->second.AddTraceData(
                      msg->signals[i].signal_dbm, tf);
        trace_maps_.find(msg->signals[i].ssid)->second.PubTraceData();
      }
      // Map volumetric data
      if (calc_volumetric_) {
        // Check if the essid is new
        if (volumetric_maps_.find(msg->signals[i].ssid) == volumetric_maps_.end()) {
          ROS_DEBUG_STREAM("New vol map " << msg->signals[i].ssid <<
            " topic " << TOPIC_WIFI_MAPPER_MAP + std::string("_") + msg->signals[i].ssid);
          volumetric_mapper::VolumetricMapper new_map_volumetric(nh_,
              TOPIC_WIFI_MAPPER_MAP + std::string("_") + msg->signals[i].ssid,
              resolution_, min_signal_strength_, max_signal_strength_, alpha_volumetric_,
              offset_signal_strength_);
          volumetric_maps_.insert({msg->signals[i].ssid, new_map_volumetric});
        }
        volumetric_maps_.find(msg->signals[i].ssid)->second.AddMapData(
                        msg->signals[i].signal_dbm, tf);
      }
    }
  }
  ...
  void WifiPubCallback(const ros::TimerEvent&) {
    // Map volumetric data
    if (calc_volumetric_) {
      // Iterate over the received wifi signals
...
int main(int argc, char **argv) {
  ros::init(argc, argv, NODE_WIFI_MAPPER);

  ros::NodeHandle nh;
  wifi_mapper::WifiMapperNode wm;
  wm.Initialize(&nh);
  ros::spin();
