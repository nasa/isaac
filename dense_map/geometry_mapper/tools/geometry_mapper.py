...
import sys, os, argparse, subprocess, re, shutil, cv2
...
def process_args(args):
    ...
    parser.add_argument('--ros_bag', dest = 'ros_bag', default = '',
                        help = 'A ROS bag with recorded nav_cam, haz_cam, and sci_cam data.')
    parser.add_argument('--sparse_map', dest = 'sparse_map', default = '',
    ...
    parser.add_argument('--nav_cam_topic', dest = 'nav_cam_topic', default = '/hw/cam_nav',
                        help = 'The nav cam topic in the bag file.')
    parser.add_argument('--haz_cam_points_topic', dest = 'haz_cam_points_topic',
                        default = '/hw/depth_haz/points',
                        help = 'The haz cam point cloud topic in the bag file.')
    parser.add_argument('--haz_cam_intensity_topic', dest = 'haz_cam_intensity_topic',
                        default = '/hw/depth_haz/extended/amplitude_int',
                        help = 'The haz cam intensity topic in the bag file.')
    parser.add_argument('--sci_cam_topic', dest = 'sci_cam_topic', default = '/hw/cam_sci',
                        help = 'The sci cam topic in the bag file.')
    parser.add_argument('--camera_type', dest = 'camera_type', default = 'all',
                        help = 'Specify which cameras to process. By default, process all. '
                        + 'Options: nav_cam, sci_cam.')
    parser.add_argument('--start', dest = 'start', default = '0.0',
                        help =  'How many seconds into the bag to start processing the data.')
    parser.add_argument('--duration', dest = 'duration', default = '-1.0',
                        help = 'For how many seconds to do the processing.')
    parser.add_argument('--sampling_spacing_seconds', dest = 'sampling_spacing_seconds',
                        default = '2',
                        help = 'Spacing to use, in seconds, between consecutive depth images in '
                        'the bag that are processed.')
    parser.add_argument('--dist_between_processed_cams', dest = 'dist_between_processed_cams',
                        default = '0.1',
    ...
    parser.add_argument('--sci_cam_timestamps', dest = 'sci_cam_timestamps',
                        default = '',
    ...
    parser.add_argument('--depth_exclude_columns', dest = 'depth_exclude_columns', default = '0',
                        help = 'Remove this many columns of data from the rectangular ' +
                        'depth image sensor at margins to avoid distortion.')
    parser.add_argument('--depth_exclude_rows', dest = 'depth_exclude_rows', default = '0',
                        help = 'Remove this many rows of data from the rectangular ' +
                        'depth image sensor at margins to avoid distortion.')
    parser.add_argument('--foreshortening_delta', dest = 'foreshortening_delta', default = '5.0',
    ...
    parser.add_argument('--median_filters', dest = 'median_filters', default = '7 0.1 25 0.1',
    ...
    parser.add_argument('--depth_hole_fill_diameter', dest = 'depth_hole_fill_diameter', default = '30',
    ...
    parser.add_argument('--max_ray_length', dest = 'max_ray_length', default = '2.0',
                        help = 'Process haz cam depth image points no further than '
                        + 'this distance from the camera.')
    parser.add_argument('--voxel_size', dest = 'voxel_size', default = '0.01',
                        help = 'When fusing the depth point clouds use a voxel of this size, '
                        + 'in meters.')
    parser.add_argument('--max_iso_times_exposure', dest = 'max_iso_times_exposure',
                        default = '5.1',
    ...
    parser.add_argument('--merge_maps', dest = 'merge_maps',
                        ...
                        'registered map.')
    parser.add_argument('--start_step', dest = 'start_step', default = '0',
                        help = 'Start processing at this step. Useful for resuming work. ' + \
                        'See the doc for options.')
    parser.add_argument('--astrobee_build_dir', dest = 'astrobee_build_dir',
                        default = astrobee_build_dir,
                        help = 'The path to the Astrobee build directory.')
    parser.add_argument('--localization_options', dest = 'localization_options',
    ...
    parser.add_argument('--use_brisk_map', dest = 'use_brisk_map', action = 'store_true',
    ...
    parser.add_argument('--save_debug_data', dest = 'save_debug_data', action = 'store_true',
...
def sanity_checks(geometry_mapper_path, batch_tsdf_path, args):
    ...
    if not os.path.isdir(args.astrobee_build_dir):
        raise Exception("Cannot find the astrobee_build directory. "
                        + "Specify it via --astrobee_build_dir.")

    if args.camera_type == 'nav_cam' and args.simulated_data:
        print("Cannot apply nav cam texture with simulated cameras " + \
...
def compute_poses_and_clouds(geometry_mapper_path, args):
    ...
    cmd = [geometry_mapper_path,
           '--ros_bag', args.ros_bag,
           '--sparse_map', args.sparse_map,
           '--output_dir', args.output_dir,
           '--nav_cam_topic', args.nav_cam_topic,
           '--haz_cam_points_topic', args.haz_cam_points_topic,
           '--haz_cam_intensity_topic', args.haz_cam_intensity_topic,
           '--sci_cam_topic', args.sci_cam_topic,
           '--camera_type', args.camera_type,
           '--start', args.start,
           '--duration', args.duration,
           '--sampling_spacing_seconds', args.sampling_spacing_seconds,
           '--dist_between_processed_cams', args.dist_between_processed_cams,
           '--max_iso_times_exposure', args.max_iso_times_exposure,
           '--depth_exclude_columns', args.depth_exclude_columns,
           '--depth_exclude_rows', args.depth_exclude_rows,
           '--foreshortening_delta', args.foreshortening_delta,
           '--depth_hole_fill_diameter', args.depth_hole_fill_diameter,
           '--median_filters', args.median_filters,
    ...
    if args.scicam_to_hazcam_timestamp_offset_override_value != "":
        cmd += ['--scicam_to_hazcam_timestamp_offset_override_value',
                args.scicam_to_hazcam_timestamp_offset_override_value]

    log_file = os.path.join(args.output_dir, 'geometry_mapper_log.txt')
    print('Compute camera poses and extract data from the bag. Writing the output log to: ' + log_file)
...
def fuse_clouds(batch_tsdf_path, mesh, args):
    ...
    print('Fusing the depth images with voxblox. Writing the output log to: ' + log_file)
    run_cmd(cmd, log_file, verbose = args.verbose)

def smoothe_mesh(input_mesh, output_mesh, args, attempt):

    smoothe_mesh_path = os.path.join(os.environ['HOME'],
    ...
    num_iterations = '1'
    smoothe_boundary = '1'
    cmd = [smoothe_mesh_path, num_iterations, args.smoothing_time, smoothe_boundary,
           input_mesh, output_mesh]

    log_file = os.path.join(args.output_dir, 'smooth_mesh_attempt_' \
                            + str(attempt) + '_log.txt')
...
def fill_holes_in_mesh(input_mesh, output_mesh, args, attempt):
    ...
    cmd = [fill_holes_path, args.max_hole_diameter,
           args.max_num_hole_edges, input_mesh, output_mesh]

    log_file = os.path.join(args.output_dir, 'fill_holes_attempt_' \
                            + str(attempt) + '_log.txt')
...
def rm_connected_components(input_mesh, output_mesh, args):
    ...
    rm_connected_components_path = os.path.join(os.environ['HOME'],
    ...
    cmd = [rm_connected_components_path, args.num_components_to_keep,
           args.num_min_faces_in_component, input_mesh, output_mesh]

    log_file = os.path.join(args.output_dir, 'rm_connected_components_log.txt')
    print('Removing small connected components from mesh. Writing the output log to: ' + log_file)
    run_cmd(cmd, log_file, verbose = args.verbose)

def simplify_mesh(input_mesh, output_mesh, args):

    simplify_mesh_path = os.path.join(os.environ['HOME'],
                                                'projects/cgal_tools/simplify_mesh')
...
def undistort_images(args, cam_type, dist_image_list, undist_dir, undist_crop_win, scale):
    
    undistort_image_path = os.path.join(args.astrobee_build_dir,
                                        'devel/lib/camera/undistort_image')
    if not os.path.exists(undistort_image_path):
        raise Exception("Cannot find the undistort_image tool: " + undistort_image_path)
    
    cmd = [undistort_image_path, '--undistorted_crop_win',  undist_crop_win,
           '--save_bgr', '--robot_camera', cam_type, '-image_list', dist_image_list,
           '--scale', str(scale), '--output_directory', undist_dir]

    log_file = os.path.join(args.output_dir, 'undist_' + cam_type + '_log.txt')
    print('Undistorting ' + cam_type + ' images. Writing the output log to: ' + log_file)
    run_cmd(cmd, log_file, verbose = args.verbose)
...
def create_texrecon_cameras(args, src_path, undist_dir, cam_type):
    ...
    cmd = ['python', cam_to_texrecon_path, '--camera_dir', args.output_dir,
           '--undistorted_image_dir', undist_dir, '--camera_type', cam_type]
           
    log_file = os.path.join(args.output_dir, 'setup_texrecon_' + cam_type + '_log.txt')
    print('Preparing texrecon cameras for ' + cam_type + '. Writing the output log to: ' + log_file)
    run_cmd(cmd, log_file, verbose = args.verbose)
...
def run_texrecon(args, src_path, mesh, undist_dir, cam_type):
    ...
    texrecon_path = os.path.join(texrecon_path,
    ...
    cmd = [texrecon_path, undist_dir, mesh, texrecon_dir, 
           '-o', 'gauss_clamping', '-d', 'view_dir_dot_face_dir',
           '--keep_unseen_faces']

    log_file = os.path.join(args.output_dir, 'texrecon_' + cam_type + '_log.txt')
    print('Running texrecon for ' + cam_type + '. Writing the output log to: ' + log_file)
    run_cmd(cmd, log_file, verbose = args.verbose)
...
def texture_mesh(src_path, cam_type, mesh, args):
    ...
    if not args.simulated_data:
        ...
        undistort_images(args, cam_type, dist_image_list, undist_dir, undist_crop_win, scale)
...
if __name__ == '__main__':
    ...
    batch_tsdf_path = os.path.join(os.environ['HOME'], 'catkin_ws/devel/lib/voxblox_ros/batch_tsdf')
    ...
    if start_step <= 9:
        ...
        if (not args.simulated_data) and (args.camera_type == 'all' or \
