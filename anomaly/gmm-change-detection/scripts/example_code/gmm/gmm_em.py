...
# 0. Create dataset
X, Y = make_blobs(
    cluster_std=1.5, n_features=3, random_state=20, n_samples=50, centers=3
)
...
class GMM:
    ...
    def run(self):
        ...
        x, y, z = np.meshgrid(
            np.sort(self.X[:, 0]), np.sort(self.X[:, 1]), np.sort(self.X[:, 2])
        )
        ...
        self.pi = (
            np.ones(self.number_of_sources) / self.number_of_sources
        )  # Are "Fractions"
        log_likelihoods = (
            []
                    ...
                    / np.sum(
                        [
                            ...
                            for pi_c, mu_c, cov_c in zip(
                                self.pi, self.mu, self.cov + self.reg_cov
                            )
                        ],
                        axis=0,
                    )
    ...
    def predict(self, Y):
        ...
        for m, c in zip(self.mu, self.cov):
            ...
            ax2.contour(
                ...
                multi_normal.pdf(self.XY).reshape(
                    len(self.X), len(self.X), len(self.X)
                ),
                colors="black",
                alpha=0.3,
            )
        ...
        for m, c in zip(self.mu, self.cov):
            print(c)
            prediction.append(
                multivariate_normal(mean=m, cov=c).pdf(Y)
                / np.sum(
                    [
