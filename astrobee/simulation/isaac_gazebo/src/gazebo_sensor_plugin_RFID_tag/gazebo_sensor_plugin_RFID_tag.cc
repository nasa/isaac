...
class GazeboSensorPluginRFIDTag : public FreeFlyerSensorPlugin {
 public:
  GazeboSensorPluginRFIDTag() :
    FreeFlyerSensorPlugin("RFID_tag", "", true) {}
 ...
 protected:
  ...
  void LoadCallback(ros::NodeHandle* nh,
    ...
    sensor_ = std::dynamic_pointer_cast<sensors::RFIDTag>(sensor);
    if (!sensor_) {
      gzerr << "GazeboSensorRFIDTag requires a valid sensor.\n";
      return;
    }

    // Setup TF2 message
    msg_.header.frame_id = "world";
    msg_.child_frame_id = sensor_->Name();
...
//   }
 ...
 private:
  ...
  sensors::RFIDTagPtr sensor_;
  physics::EntityPtr parent_;
  gazebo::transport::SubscriberPtr update_;
  int id_ = 0;
};

GZ_REGISTER_SENSOR_PLUGIN(GazeboSensorPluginRFIDTag)
