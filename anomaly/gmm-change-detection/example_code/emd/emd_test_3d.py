...
np.random.seed(0)
n_samples = 1000
X1 = np.concatenate([np.random.normal(-1, 1, int(0.3 * n_samples)),
                         np.random.normal(5, 1, int(0.7 * n_samples))]).reshape(-1, 1)
X2 = np.concatenate([np.random.normal(0, 2, int(0.4 * n_samples)),
                         np.random.normal(3, 1, int(0.6 * n_samples))]).reshape(-1, 1)
...
emd = wasserstein_distance(means1, means2, u_weights=weights1, v_weights=weights2, metric='euclidean')
