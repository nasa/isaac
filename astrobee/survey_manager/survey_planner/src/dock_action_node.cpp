...
class DockAction : public plansys2::ActionExecutorClient {
 public:
  DockAction(ros::NodeHandle nh, const std::string& action, const std::chrono::nanoseconds& rate)
 ...
 protected:
  void do_work() {
    ...
    if (get_arguments().size() > 2) {
      ...
      finish(false, 1.0, "Not enough arguments for [DOCK] command");
    }

    // Start process if not started yet
    if (progress_ == 0.0) {
      std::string command =
        "rosrun survey_planner command_astrobee " + robot_name_ + " dock " + towards + " " + from + " run1";
    ...
    if (fgets(buffer, 1028, process_pipe_) != NULL) {
      ...
      if (progress_ < 1.0) {
        progress_ += 0.02;
        send_feedback(progress_, "Dock and Inspect running");
      }

      std::cout << "\t ** [Dock] Robot " << robot_name_ << " moving from " << from << " towards " << towards
      ...
      if (status == 0) {
        ...
        finish(true, 1.0, "Dock completed");
      } else {
        std::cout << "Command terminated with status fail:  " << status << std::endl;
        finish(false, 1.0, "Dock terminated by signal");
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  auto action_node = std::make_shared<plansys2_actions::DockAction>(nh, "dock",  std::chrono::seconds(1));
